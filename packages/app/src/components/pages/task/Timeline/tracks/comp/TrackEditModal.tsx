...
interface TrackEditModalProps {
  ...
  initialEndMs: number;
  /** 额外字段展示（如置信度/box_y、原文/语言），插槽自由扩展 */
  extraFields?: () => JSX.Element;
  /** 保存回调，负责写盘；成功返回后自动关闭弹窗 */
  onSave: (values: { text: string; startMs: number; endMs: number }) => Promise<void> | void;
}

/** 通用轨道编辑弹窗：文本 + 开始/结束 + 可插拔额外字段 */
export function TrackEditModal(props: TrackEditModalProps) {
  ...
  const onSave = async () => {
    await props.onSave({ text: text(), startMs: startMs(), endMs: endMs() });
    closeModal();
  };
