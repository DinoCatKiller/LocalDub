...
import { set_resumeFrom, setRunningStage, setViewingTab, StageTab, use_resumeFrom, useRunningStage, useViewingTab } from "./taskControlPanelStore";
...
export const TaskControlPanel = (p: {
  ...
  const resume_task = useMutation(() => client.resume_task.mutationOptions({
    onSuccess: () => {
  ...
  return (
    <div class="w-100 min-w-40 border-r flex text-muted-foreground text-sm overflow-hidden">
      <Tabs value={viewingTab()} onChange={(value) => setViewingTab(value as StageTab)} class="w-full" orientation="vertical">
        {/* 左侧 tab 列表 */}
        <TabsList class="w-30" >
          <For each={tabs()}>{(tab) => {
                  ...
                  <ContextMenuContent>
                    <Show when={tab !== 'root'}>
                      <ContextMenuItem onSelect={() =>
                        handleResumeFrom(tab)}>从这一阶段继续运行
                      </ContextMenuItem>
                    </Show>
