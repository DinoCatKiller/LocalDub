...
export const setViewingTab = (tab?: StageTab | null) => taskControlPanelStore.setState(state => ({
  ...state,
  viewingTab: tab ?? 'root'
}))

export const useRunningStage = () => useSelector(taskControlPanelStore, (state) => state.runningStage);
export const setRunningStage = (stage?: StageTab | null) => taskControlPanelStore.setState(state => ({
  ...state,
  runningStage: stage ?? 'root'
}))

export const use_resumeFrom = () => useSelector(taskControlPanelStore, (state) => state.resumeFrom)
export const set_resumeFrom = (stage?: StageName | null) => taskControlPanelStore.setState(state => ({
  ...state,
  resumeFrom: stage
}))
