  ...
  async function translateBatch(batchTexts: string[], attempt = 0): Promise<string[]> {
    ...
    try {
      ...
      if (results.length < batchTexts.length) {
        ...
        if (attempt < 2) {
          emitLog(taskDir, `[WARN] ${msg}, retrying...`);
          throw new Error(msg);
        }
        const missing = batchTexts.length - results.length;
        emitLog(
          taskDir,
          `[WARN] [Translate] batch produced ${results.length}/${batchTexts.length}, padding ${missing} empty item(s) after retries`,
        );
        while (results.length < batchTexts.length) results.push("");
      }
      return results;
    } catch (e: any) {
