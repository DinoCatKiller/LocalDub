...
import { stageSfOcr } from "./sf_ocr/ocr";
import { stageSfOcrFix } from "./sf_ocr/ocr_fix";
import { stageAsrOcrPre } from "./04_asr_ocr/ocr_pre";
import { stageAsrOcr } from "./04_asr_ocr/ocr";
...
export const STAGE_HANDLERS: Record<string, StageHandler> = {
  ...
  sf_ocr: async (sp) => await stageSfOcr(readCtx(sp)),
  sf_ocr_fix: async (sp) => await stageSfOcrFix(readCtx(sp)),
